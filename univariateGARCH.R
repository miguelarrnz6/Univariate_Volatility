...
plot.ts(residuals(MSFT.garch11.fit, standardize=TRUE), ylab="e(t)", col="blue")
abline(h=0)
...
garch11t.spec <- ugarchspec(variance.model = list(garchOrder=c(1,1)), 
                            mean.model = list(armaOrder=c(0,0)),
                            distribution.model = "std")
MSFT.garch11t.fit <- ugarchfit(spec=garch11t.spec, data=MSFT.ret,
                               solver.control=list(trace = 1))                          

coef(MSFT.garch11t.fit)

plot(MSFT.garch11t.fit)

# M-GARCH Garch in Mean

garch11m.spec <- ugarchspec(variance.model = list(garchOrder=c(1,1)), 
                           mean.model = list(armaOrder=c(0,0), 
                                             archm = TRUE))
MSFT.garch11m.fit <- ugarchfit(spec=garch11m.spec, data=MSFT.ret,
                              solver.control=list(trace = 1))                          

coef(MSFT.garch11m.fit)
